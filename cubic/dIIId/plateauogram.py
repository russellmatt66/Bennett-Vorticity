'''
Histogram of the front and wake accuracies for the best solutions to the DIIID 2005 edge pedestal obtained in best_DIIID_plateau.py
'''
import sys
import pathlib
...
front_rrmses_root_str = "best_front_rrmses"
wake_rrmses_root_str = "best_wake_rrmses"
...
for idx in range(platidx_low, platidx_high + 1):
    ...
    bfr_df = pd.read_csv(front_rrmses_filename, header=None, names=['rrmse'])
    bwr_df = pd.read_csv(wake_rrmses_filename, header=None, names=['rrmse'])
    ...
    N = bwr_rrmses.size # Both arrays come from same sweep
    ...
    ax[0].set_title(f'Histogram of RRMSE for Best Wake Solutions to DIIID 2005 Edge Pedestal \n rplatidx = {idx}, N={N}')
    ax[0].legend()

    ax[1].hist(bfr_rrmses, bins=bins, alpha=0.5)
    ax[1].set_xlabel('Range-Normalized Relative Root Mean Squared Error (RRMSE)')
    ax[1].set_ylabel('Frequency')
    ax[1].set_title(f'Histogram of RRMSE for Best Front Solutions to DIIID 2005 Edge Pedestal Sweep \n rplatidx = {idx}, N={N}')
    ax[1].legend()
