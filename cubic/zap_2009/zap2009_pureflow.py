...
for i in range(len(uz_df_list)):
    uz0_temp_pos = cpfm.root_solve_chi2_pure(uedge_pos[i], n0, rp_pos[i], Tp)
    uz0_temp_neg = cpfm.root_solve_chi2_pure(uedge_neg[i], n0, rp_neg[i], Tp)
    ...
    for uz0 in uz0_temp_pos:
        ...
        uzpos_fit = cpfm.uz_chi2cubic_pure(cbt_pos, np.abs(uz0), r_uzpos[i])
    ...
    for uz0 in uz0_temp_neg:
        ...
        uzneg_fit = cpfm.uz_chi2cubic_pure(cbt_neg, np.abs(uz0), r_uzneg[i])
