...
for i, uz0 in enumerate(uz0_roots):
    ...
    # drift_ax.loglog(r, uzs[i], label=f'Diamagnetic drift (velocity profile)')
    # drift_ax.loglog(r, vg_wf[i], label=f'Waterfall gravitational (radial) drift')
    # drift_ax.loglog(r, v_resistive[i], label=f'Resistive (radial) drift')
    # drift_ax.loglog(r, vg_shell[i], label=f'Shell gravitational (axial) drift')
    ...
    min_speed = min(vmin_resistive[i], vmin_wf[i], vmin_shell[i])
    max_speed = max(vpeak_resistive[i], vpeak_wf[i], vpeak_shell[i])
