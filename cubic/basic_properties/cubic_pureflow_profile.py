...
Why does this file not use the library? Because it's copied from here: https://github.com/russellmatt66/imhd-CUDA/blob/main/python/bennett-vorticity/temp-rcubed/temp-rccubed_shear_flow.py
'''
# Normalized profiles
...
shear_tilde = 2 * ((phi) / (phi + 1)**2) - ((phi**2) / (phi + 1)**3) # normalized shear flow - shear_tilde = d(uz_tilde)/d(phi)
...
flow_ax.set_title('Normalized Flow Profile $\\tilde{u}_{z} = \\frac{u_z}{u_{z0}}$ of an $r^{3}$ Bennett-Shumlak Vortex')
...
shear_ax.set_title('Normalized Shear Flow Profile $\\frac{d\\tilde{u}_{z}}{d\phi}$ of an $r^{3}$ Bennett-Shumlak Vortex')
